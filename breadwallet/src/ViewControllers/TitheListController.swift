...
let tithesHeaderHeight: CGFloat = 152.0
let tithesFooterHeight: CGFloat = 0.0

class TitheListController : UIViewController, Subscriber {
    ...
    init(currency: CurrencyDef, walletManager: WalletManager) {
        ...
        self.headerView = TitheHeaderView(currency: currency)
        super.init(nibName: nil, bundle: nil)
        self.tithesTableView = TitheTableViewController(currency: currency, walletManager: walletManager, didSelectTithe: didSelectTithe )
    ...
    private let headerView: TitheHeaderView
    private let transitionDelegate = ModalTransitionDelegate(type: .transactionDetail)
    private var tithesTableView: TitheTableViewController!
    private var isLoginRequired = false
    private let searchHeaderview: TitheSearchHeaderView = {
        let view = TitheSearchHeaderView()
        view.isHidden = true
        return view
    }()
    ...
    private func setInitialData() {
        ...
        searchHeaderview.didChangeFilters = { [weak self] filters in
            self?.tithesTableView.filters = filters
        }
    }

    private func addTransactionsView() {
        view.backgroundColor = .whiteBackground
        addChildViewController(tithesTableView, layout: {
            if #available(iOS 11.0, *) {
                tithesTableView.view.constrain([
                tithesTableView.view.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor),
                tithesTableView.view.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor),
                tithesTableView.view.leadingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.leadingAnchor),
                tithesTableView.view.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor),
                ])
            } else {
                tithesTableView.view.constrain(toSuperviewEdges: nil)
            }
        })
    }
    ...
    private func didSelectTithe(tithe: [TitheRowModel], selectedIndex: Int) -> Void {
        let titheRow = tithe[selectedIndex]
        let paymentReqString = PaymentRequest.requestString(withAddress: titheRow.address, forAmount: 0, currency: Currencies.btc)
        var paymentRequest = PaymentRequest(string: paymentReqString, currency: Currencies.btc)!
        paymentRequest.label = "Tithe for \(titheRow.name)"
            
        Store.perform(action: RootModalActions.Present(modal: .sendTithe(paymentRequest: paymentRequest)))
    }
    ...
    private func showSearchHeaderView() {
        ...
        var contentInset = self.tithesTableView.tableView.contentInset
        var contentOffset = self.tithesTableView.tableView.contentOffset
        contentInset.top += navBarHeight
        contentOffset.y -= navBarHeight
        self.tithesTableView.tableView.contentInset = contentInset
        self.tithesTableView.tableView.contentOffset = contentOffset
